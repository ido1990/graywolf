...
// Always report a zero htstamp.
//
// The C call above still writes into the 16-byte overflow-safe buffer, so the
// t64 stack-smash fix is unchanged; we simply discard the decoded value and
// return (0, 0). Two reasons:
//
//  1. graywolf (and any cpal consumer that ignores the callback `info`
//     timestamps) never reads this value, so correctness is irrelevant.
//
//  2. Returning (0, 0) makes cpal's ALSA backend select its *software*
//     timestamp fallback (it probes `get_htstamp()` once at stream build and
//     uses `Instant`-since-creation when it is zero). The hardware path it
//     would otherwise take computes `ts.tv_sec * 1_000_000_000` in i32, which
//     overflows on 32-bit for any uptime over ~2 s and reports
//     "get_htstamp ... was earlier than get_trigger_htstamp" every period --
//     a crash-rebuild loop that silences audio on 32-bit ARM t64 (Pi Zero).
//
// On a legacy time32 system this is equally fine: the value was already lossy
// there and nobody who hits this loop consumes it.
fn decode_htstamp(_buf: &Htstamp64) -> timespec {
...
// decode_htstamp deliberately discards the buffer and reports (0, 0) so cpal's
// ALSA backend selects its software-timestamp fallback instead of the hardware
// path (which overflows i32 on 32-bit). See the comment on `decode_htstamp`.
#[test]
fn htstamp_decode_reports_zero() {
    let mut raw = [0u8; 16];
    ...
    assert_eq!(ts.tv_sec as i64, 0);
    assert_eq!(ts.tv_nsec as i32, 0);
}
